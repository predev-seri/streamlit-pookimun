...
st.set_page_config (
    page_title="푸키먼",
    page_icon="./images/monsterball - 복사본.png"
)

st.title("푸키먼")
st.markdown("**대학교**에 합격하고 싶다 푸키먼들아!")
...
with st.form(key="form"):
    ...
    if submit:
        if not name:
            ...
            st.error("포켓몬의 속성을 적어도 한 개 이상 선택해주세요.")
        else:
            st.success("포켓몬을 추가할 수 있습니다!")
            st.session_state.pokemons.append({
                "name" : name,
                "types" : types,
                "image_url" : image_url if image_url else "./images/default - 복사본.png"
            })
